...
const Restaurant = () => {
  ...
  const fetchMyRestaurant = async () => {
    try {
      ...
      // Agar error aaye toh restaurant null hi rahega
      setRestaurant(null);
    } finally {
      setLoading(false);
    }
        ...
        <div className="flex border-b">
          {[
            ...
            <button
              ...
              className={`flex-1 px-4 py-3 text-sm font-medium transition ${tab === t.key ? "border-b-2 bprder-red-500 text-red-500": "text-gray-500 hover:text-gray-700"}`}
            >
              {t.label}
            </button>
          ))}
